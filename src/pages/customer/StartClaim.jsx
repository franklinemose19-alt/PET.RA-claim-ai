...
// On submit: insert claim row, insert claim_media rows, then call
// /api/analyze-claim. The AI call runs AFTER the claim is safely saved —
// if AI analysis fails, the claim still exists and is just marked for
// manual review (handled inside analyze-claim.js itself).
...
export default function StartClaim() {
  ...
  // Load customer's connected policies
  useEffect(() => {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // 2. Upload each photo, then insert claim_media rows
      ...
      // 3. Trigger AI analysis — fire and don't block navigation on its result.
      //    If this fails, analyze-claim.js's own error handling still leaves
      //    the claim in a valid state (manual_review_required), so we don't
      //    need to retry here.
      ...
      // 4. Notify customer their claim was submitted
