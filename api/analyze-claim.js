...
//
// Triggered after a claim is submitted (called from the frontend right after
// the claim + media rows are inserted, or via a Supabase DB webhook on INSERT
// into `claims` — webhook approach is cleaner long-term but needs Supabase
// project config, so wiring it as a direct call from the frontend for now).
//
...
export default async function handler(req, res) {
  ...
  try {
    // 1. Fetch claim + metadata + policy info
    ...
    // 2. Fetch media rows for this claim (capped at MAX_IMAGES)
    ...
    if (!mediaRows || mediaRows.length === 0) {
      // No media — store a manual-review result rather than calling the model at all
    ...
    // 3. Get signed URLs and download images as base64 for the vision call
    const imageBlocks = [];
    for (const row of mediaRows) {
      const { data: signed, error: signError } = await supabaseAdmin.storage
        .from('claim-evidence')
        .createSignedUrl(row.storage_path, 300); // 5 min, only needed for this call
    ...
    // 4. Build the metadata text block
    const metadataText = `
...
Photo angle labels: ${mediaRows.map(m => m.angle_label || 'unlabeled').join(', ')}
    ...
    // 6. Parse + validate JSON, with one retry-via-cleanup before falling back
    ...
    // 7. If high risk or fraud flagged, mark claim status for adjuster priority
    //    (never auto-approve/reject — only ever flag for human review)
    if (validated.fraud_flag || validated.risk_score >= 70) {
      await supabaseAdmin
        .from('claims')
        .update({ status: 'under_review' })
        .eq('id', claim_id)
        .eq('status', 'submitted'); // only bump if still in initial state
    }

    return res.status(200).json({ ok: true, result: validated });
  } catch (err) {
    console.error('analyze-claim error:', err);
    // Last-resort fallback: never leave a claim with no ai_results row at all
    try {
